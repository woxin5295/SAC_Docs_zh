...
并进行电压放大。因而此阶段的输入单位是运动物理量的单位（比如 m/s），
输出单位是伏特（V）。
...
已知该仪器在 $$f_s=1 Hz$$ 时的增益为 $$Sd=150 V/m/s^2$$，
即若仪器接收到 1 Hz 频率的加速度 $$1 m/s^2$$， 则仪器的输出电压为 150 V
...
输入的单位是伏特V，输出单位是counts 。这个阶段，所有频段
有相同的振幅响应，即只存在一个放大系数，同时可能存在一个时间延迟。

假设模数转换器是 $$m$$ 位，电压范围是 $$\pm n$$，那么 $$1 counts=\frac{2n}{2^m} V$$，
电压也能精确到 $$\frac{2n}{2^m} V$$。
例如， 24位模数转换器的输入电压范围是$$\pm 20 V$$，则输出的最大范围是 $$\pm 2^{23}$$。
因而ADC的放大系数为
$$S_d = 2^24/40 counts/V = 4.1943e5 counts/V$$
即若ADC接收到的输入电压为1 V，则其输出为 $$4.1983\times 10^5$$。
结合地震仪的放大系数可知，频率为 1 Hz 的 $$1 m/s^2$$ 的地面运动加速度在ADC的输出为：

$$1 m/s^2 * 150 V/m/s^2 * 4.1943e5 counts/V = 6.29145e7 counts$$

另一方面，由于ADC的输入电压的上限为 20 V，因而仪器所能记录的
最大加速度为 $$20/150=0.13 m/s^2$$。
...
FIR滤波器，而FIR滤波器的振幅响应函数可以认为在全频段内为1，因而这个阶段只
...
因子 $$A_0$$ 以及零极点信息；第二个阶段以及第三个阶段都只需要给出
放大系数$$Sd_{2}$$ 和 $$Sd_3$$ 。
