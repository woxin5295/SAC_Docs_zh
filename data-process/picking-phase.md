...
变量的设置方式参考 [在Linux下安装SAC](/introduction/linux-install.md)  一节。

### qdp off

SAC在默认情况下会打开快速绘图选项，即 `qdp on`。关于 `qdp`，
可以参考 [图像外观](/graphics/plot-appearance.md) 一节以及命令
...
绘图选项，这样相对比较麻烦，可以使用 [SAC初始化脚本](/tricks-and-traps/init-macro.md)
一节中介绍的方法使得每次SAC启动时自动关闭快速绘图选项。
...
除了上面介绍的若干ppk命令之外，还有很多其他ppk命令。 下表列出了ppk模式下的所有命令，
